...
function exec(config, cb) {
    ...
    if (customData) {
        ...
        customItems = common.handleDependence(rootPath, customItems);
...
function compile (customItems, data, cb) {
    // compile
...
function getOfflineJson (customItems) {
    ...
    for (let i = 0; i < customItems.length; i++) {
        ...
        let dataPath = path.resolve(rootPath, src, template, customItems[i], fileName + '.json');
