...
const common = {
    ...
    handleDependence: function (rootPath, tpls) {
        ...
        for (var i = 0; i < tpls.length; i++) {
            let item = tpls[i];
            let extPth = path.resolve(rootPath, src, item);
            let pth = path.resolve(extPth, path.basename(item) + '.mustache');
            if (common.existsFile(pth)) {
                self.handleContent(self.readFile(pth), tpls);
                results.push(item);
            } else if (!common.existsFile(extPth)) {
               cli.warn('The "' + item + '" extension does not exist!');
            } else {
                cli.warn('The "' + item + '" is not customization extension!');
            }
        }
        return results;
    },
