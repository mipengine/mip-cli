        ...
        while ((results = reg.exec(content)) != null)  {
            if (results && results.length > 0) {
                ...
                if (!common.isHTML(name)
                    ...
                    let tplPath = path.resolve(rootPath, dist, getCatalog(pth), file, pct.version, file + '.html');
...
function getCatalog (pth) {
    let catalog;
    if (pth.match(path.resolve(rootPath, src, template))) {
        catalog = template;
    } else if (pth.match(path.resolve(rootPath, src, cm))) {
        catalog = cm;
    }
    return catalog;
}
