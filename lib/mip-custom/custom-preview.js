...
function exec(config, cb) {
    ...
    handleFile(filePath);
    compile(customItems, cb);
}

function getJson (customItems) {
    let content = '';
    let data = {
        "status": {
            "errorno": 0,
    ...
    for (let i = 0; i < customItems.length; i++) {
        ...
        let dataPath = path.resolve(rootPath, src, customItems[i], fileName + '.json');
        obj.tpldata = JSON.parse(common.readFile(dataPath));
        data.data.template.push(obj);
    }
    return data;
}

function compile (customItems, cb) {
    // compile
