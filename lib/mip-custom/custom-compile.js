...
function getOfflineJson (customItems) {
    ...
    let config = common.readFile(path.resolve(rootPath, 'mip-require.config.js'));
    try {
        config = JSON.parse(config);
    } catch (e) {
        cli.error('JSON parsing failed in mip-require.config.js!');
        return;
    }
    if (config.paths) {
        ...
        for (let key in pths) {
            ...
            let filePath = path.resolve(rootPath, dist, pths[key]);
            pths[key] = '/' + path.relative(configPath, filePath);
        }
    }
