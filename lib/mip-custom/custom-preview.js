...
function exec(config, cb) {
    ...
    let conf = common.readFile(path.resolve(rootPath, 'conf/mip-conf.config.json'));
    try {
        conf = JSON.parse(conf);
    } catch (e) {
        cli.warn("mip-require.config.json is not legal json!");
    }
    let disableCompile = conf.disableCompile;
